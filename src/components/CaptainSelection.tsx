...
import { ChevronsRight, Coins, Flag } from "lucide-react";
...
const CaptainSelection: React.FC = () => {
  ...
  const handleSelectCaptainForTeam = (playerId: string, teamId: string) => {
    ...
    toast({
      ...
      description: `New captain has been selected for ${
        teamId === "team-alpha" ? "Team Alpha" : "Team Beta"
      }`,
    });
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {teamAlpha.players.map((player) => (
                  <PlayerCard
                    ...
                    onSelectCaptain={(id) =>
                      handleSelectCaptainForTeam(id, "team-alpha")
                    }
                  />
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {teamBeta.players.map((player) => (
                  <PlayerCard
                    ...
                    isCaptainSelectionMode={true}
                    onSelectCaptain={(id) =>
                      handleSelectCaptainForTeam(id, "team-beta")
                    }
                  />
                ))}
              </div>
